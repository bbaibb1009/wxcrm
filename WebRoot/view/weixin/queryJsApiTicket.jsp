...
<html>
  ...
  <body>
  	...
  	<div class="content">
  	<f:form action="${pageContext.request.contextPath}/weixin/queryJsApiTicket" onsubmit="return false;">
  	<div><h1>当前可用jsApiTicket:<span style="color:red;">${jsApiTikcet}</span></h1></div>
  	<div><h1>过期时间:<span style="color:red;">${jsApiTikcetExpire}</span></h1></div>
  	<div><input type="button" value="手动获取jsApiTicket" onclick="javascript:getJsApiTicket();"/></div>
  	...
  	<table cellpadding="0" cellspacing="0" border="1" width="100%">
    	...
    	<tr>
			<td>${accessToken.WJT_ID}</td>
			<td>${accessToken.WEC_APP_NAME}</td>
			<td>${accessToken.WJT_JSAPI_TICKET}</td>
    		<td>${accessToken.WJT_EXPIRES_IN}</td>
    		<td>${accessToken.WJT_CREAT_TIME}</td>
    		<td>
    		<c:if test="${accessToken.WJT_STATUS==1}">可用</c:if>
    		<c:if test="${accessToken.WJT_STATUS==0}">不可用</c:if>
    		</td>
    	</tr>
    	</c:forEach>
    </table>
