...
<html>
  ...
  <body>
  	...
  	<f:form action="${pageContext.request.contextPath}/weixin/queryWeixinEnterBySite/${command.wecEnterId}" onsubmit="return false;">
  	<f:hidden path="wecId"/>
  	<f:hidden path="wecEnterId"/>
  	...
  	<input type="button" value="添加" onclick="window.location.href = '${pageContext.request.contextPath}/weixin/toAddWeiEnterByE/${command.wecEnterId}';"/>
  	<input type="button" value="删除" onclick="delChk('wecIds','${pageContext.request.contextPath}/weixin/delWeiEnterByE/${command.wecEnterId}');"/>
  	<table cellpadding="0" cellspacing="0" border="1" width="100%">
    	<tr>
    		<th width="1%"></th>
    		<th width="3%">名称</th>
    	...
    	<c:forEach var="weiEnter" items="${pageResult.resultList}">
	    	<tr>
	    		...
	    		<td>
	    			<c:if test="${weiEnter.WEC_CUS_TYPE==2}">${weiEnter.CUE_NAME}</c:if>
	    			<c:if test="${weiEnter.WEC_CUS_TYPE==1}">内部</c:if>
	    		</td>
	    		<td>
	    			<a href="javascript:toUpd('wecId','${weiEnter.WEC_ID}','${pageContext.request.contextPath}/weixin/toUpdWeiEnterByE')">修改</a>|
	    			...
	    			|<a href="${pageContext.request.contextPath}/weixine/queryWeiEnterCustByApp/${weiEnter.WEC_ENTER_ID}">客户</a>|
