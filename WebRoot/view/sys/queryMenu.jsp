...
<html>
  <head>
	...
	<script type="text/javascript" charset="utf-8" src="${pageContext.request.contextPath}/view/sys/js/menu.js?v=${applicationScope.sysStartUpTime}"></script>
  ...
  <body>
  	...
  	<div class="content">
  	<f:form action="${pageContext.request.contextPath}/menu/queryMenu" onsubmit="return false;">
  	<f:hidden path="wmeId"/>
  	<input type="button" value="添加" onclick="window.location.href='${pageContext.request.contextPath}/menu/toAddMenu';"/>
  	<input type="button" value="删除" onclick="delChk('menuIds', '${pageContext.request.contextPath}/menu/delMenu');"/>
 	<table cellpadding="0" cellspacing="0" border="1" width="100%">
    	...
    	<c:forEach var="menu" items="${pageResult.resultList}">
	    	<tr>
	    		...
	    		<td>${menu.WME_NAME}</td>
	    		<td>${menu.WME_URL}</td>
	    		<td>${menu.WME_LEVEL}级菜单</td>
	    		<td>${menu.WME_ORDER}</td>
	    		<td>${menu.parent_menu}</td>
	    		<td>
	    			<a href="javascript:toUpd('wmeId', ${menu.WME_ID}, '${pageContext.request.contextPath}/menu/toUpdMenu')">修改</a>
	    		</td>
	    	</tr>
    	</c:forEach>
    </table>
